...
extern s32 gThermShouldDisable; // true when test disables thermals after THERM_ENABLE_DURATION
...
void therm_8034695C(void) {
    ...
    for (i = 0; i < gThermalCount; i++) {
        therm = &gThermals[i];
        levlTher = &gLevelTHER[i];
        // for the first 10 seconds after 4 minutes, reduce the strength of thermal, if test calls for it
