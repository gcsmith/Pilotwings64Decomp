...
STATIC_FUNC void splashDrawRipple(SplashRipple* ripple) {
    ...
    uvVtxBeginPoly();
    nSize = -pSize;
    uvVtx(nSize, nSize, 0, -0x400, -0x400, 0xFF, 0xFF, 0xFF, alpha);
    uvVtx(pSize, nSize, 0, 0x400, -0x400, 0xFF, 0xFF, 0xFF, alpha);
    uvVtx(pSize, pSize, 0, 0x400, 0x400, 0xFF, 0xFF, 0xFF, alpha);
    uvVtx(nSize, pSize, 0, -0x400, 0x400, 0xFF, 0xFF, 0xFF, alpha);
    uvVtxEndPoly();
    uvGfxMtxViewPop();
}
...
void splashAdd(Mtx4F* pos, f32 size) {
    ...
    for (i = 0; i < ARRAY_COUNT(effect->fxIds); i++) {
        ...
        if (effect->fxIds[i] != 0xFF) {
            ...
            uvFxProps(effect->fxIds[i], FX_PROP_1(sSplashRippleParams[i].unkC), FX_PROP_3(size * 0.6f, size * 0.6f, size), FX_PROP_END);
            if (i % 2) {
